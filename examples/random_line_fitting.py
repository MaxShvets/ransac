...
def conduct_random_experiment():
    ...
    max_trials_amount = compute_trials_amount(inlier_probability, min_model_size)
    ...
    def random_error():
        ...
        if is_error:
            return tolerance * random.uniform(2, 20) * random.choice([-1, 1])
        else:
    ...
    fit = ransac_fit(points, LineModel, min_model_size, max_trials_amount, tolerance, min_accepted_size)

    return {
        "fit": fit,
        "real_params": (intercept, slope),
        "points": points,
        "fitting_model_found": min_accepted_size <= len(fit.get_points())
    }
...
def calculate_fit_wellness(result):
    ...
    return (fitted_intercept - real_intercept)**2 + (fitted_slope - fitted_slope)**2
...
def conduct_random_experiment_series():
    ...
    worst_fit_result_index = np.argmax(map(calculate_fit_wellness, results))
    ...
    plot_results(worst_fit_result["points"], worst_fit.get_points(), worst_fit.get_params())
